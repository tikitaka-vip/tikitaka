...
// Monkey: generate random predictions for a player
function generateMonkeyPrediction() {
...
function ensureMonkeyExists(groupId) {
  ...
  if (!monkey) {
    ...
    for (const m of matches) {
      const p = generateMonkeyPrediction();
      insert.run(monkey.id, m.id, p.score_a, p.score_b);
    }

    // Monkey tournament predictions
...
app.get('/join/:code', (req, res) => {
  ...
  const groupName = group ? group.name : 'ניחושי מונדיאל 2026';
  ...
  res.send(`<!DOCTYPE html><html lang="he" dir="rtl"><head>
    <meta charset="UTF-8">
    <title>הצטרפו ל${groupName} — TikiTaka</title>
    <meta property="og:title" content="הצטרפו ל${groupName} — ניחושי מונדיאל 2026">
    <meta property="og:description" content="${memberCount} חברים כבר מנחשים. הצטרפו, נצחו את הקוף! 🐒⚽">
