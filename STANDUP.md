...
- **Verified live on prod.** Triggered a fetch: #74 backfilled to pens 3-4, #75 to pens 2-3 (both match ESPN). **R16 #90 now correctly reads קנדה v מרוקו** (Canada + pens-winner Morocco); remaining R16 slots correctly stay TBD pending unplayed R32 parents (#77/#78/...). Pre-deploy: end-to-end test on a throwaway DB (pens tie-break to either side, decisive results, idempotency, no-pens-stays-undecided) + a booted test server confirming the ESPN backfill orientation.
- **Next:** as the rest of R32 lands, R16 89/96 should fill automatically (including any further shootouts) with no operator action. Builder queue otherwise empty (all tickets review/done).
